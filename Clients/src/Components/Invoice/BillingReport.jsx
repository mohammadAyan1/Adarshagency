...
import Loader from "../Loader";
import { useNavigate, useParams } from "react-router-dom";
import { createInvoice, updateInvoice, fetchInvoiceById } from "../../redux/features/product-bill/invoiceThunks";
import { useDispatch } from "react-redux";
...
function BillingReport() {
  ...
  const handleSubmit = async (shouldNavigate = true) => {
    ...
    try {
      ...
      if (id) {
        invoice = await dispatch(updateInvoice({ id, invoiceData: finalData })).unwrap();
        toast.success("Invoice updated successfully!");
      } else {
        invoice = await dispatch(createInvoice(finalData)).unwrap();
        toast.success("Invoice created successfully!");
